...
from PyQt5.QtCore import (
    Qt, QSize, QTimer, QStringListModel, QUrl, QPointF, pyqtSignal,
)
from PyQt5.QtGui import (
    QBrush, QColor, QFont, QIcon, QPainter, QPixmap, QPolygonF,
)
from PyQt5.QtWidgets import (
    QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
    QLineEdit, QPushButton, QListWidget, QListWidgetItem, QLabel,
    QComboBox, QCheckBox, QFileDialog, QTextEdit, QSplitter,
    QMessageBox, QGroupBox, QProgressBar, QStackedWidget, QCompleter,
...
class AudioPlayerBar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.autoplay_cb = QCheckBox("자동재생")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        splitter.setSizes([1000, 440])  # ~70 / 30
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        self.search_input.setObjectName("heroInput")
        ...
        # action row
        btns = QHBoxLayout()
        self.play_results_btn = QPushButton("▶ 브라우저")
        ...
        self.uncheck_all_btn = QPushButton("☐ 전체 해제")
        self.uncheck_all_btn.clicked.connect(lambda: self._set_all_checked(False))
        self.add_to_queue_btn = QPushButton("✚ 대기열에 추가")
        self.add_to_queue_btn.setObjectName("accentBtn")
        ...
        # loading view — centered "검색 중…" with a small spinner
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self.add_url_btn.setObjectName("primaryBtn")
    ...
    def _build_options_box(self) -> QGroupBox:
        ...
        # row 1: 저장형식  ☑비디오 ☑오디오
        self.fmt_video = QCheckBox("비디오")
        self.fmt_audio = QCheckBox("오디오")
        ...
        self.rename_input.setPlaceholderText("단일 항목일 때만 적용")
        v.addLayout(self._row("파일명 변경", self.rename_input))

        # row 5: 일괄 파일명 추가  [input]
    ...
    def _build_action_box(self) -> QGroupBox:
        ...
        self.log_view.setFixedHeight(56)
    ...
    def _setup_autocomplete(self):
        ...
        self._completer.setModel(self._suggest_model)
        self._completer.setCaseSensitivity(Qt.CaseInsensitive)
        self._completer.setCompletionMode(QCompleter.UnfilteredPopupCompletion)
        self._completer.setMaxVisibleItems(10)
        # Make the popup text match the rest of the UI (Qt's default popup font
        # is smaller than the QSS `*` rule for some platform styles).
    ...
    def _on_suggestions(self, items):
        if not items:
            return
    ...
    def _play_video(self, info: dict):
        ...
        if not vid or self._audio_player is None:
            QMessageBox.warning(self, "재생", "오디오 모듈을 사용할 수 없습니다.")
            return
    ...
    def _tick_spinner(self):
        ...
        self.loading_label.setText(f"{ch}  검색 중…")
        # Update sentinel if it's currently in 'loading' state
        sentinel = self._sentinel_item()
        if sentinel is not None and sentinel.data(SENTINEL_ROLE) == 'loading':
            sentinel.setText(f"{ch}  결과 불러오는 중…")
        # Update player bar play button if audio is being downloaded
    ...
    # -------------------------------------------------- search & results
    def do_search(self):
        # close the autocomplete popup as soon as the user commits to a search
        if hasattr(self, '_completer'):
            popup = self._completer.popup()
            if popup is not None:
                popup.hide()
        # also stop any pending suggestion fetch — its result would just re-open it
    ...
    def _on_search_error(self, err: str):
        ...
        QMessageBox.warning(self, "검색 오류", err)
    ...
    def play_results_selected(self):
        ...
        if not item:
            QMessageBox.information(self, "재생", "먼저 항목을 선택해주세요.")
            return
        idx = item.data(Qt.UserRole)
    ...
    def add_checked_to_queue(self):
        ...
        if added == 0:
            QMessageBox.information(self, "대기열", "체크된 항목이 없거나 모두 이미 추가되었습니다.")
    ...
    def add_url(self):
        ...
        if 'youtube.com' not in url and 'youtu.be' not in url:
            QMessageBox.warning(self, "잘못된 URL", "유튜브 URL을 입력해주세요.")
            return
    ...
    def _on_url_error(self, err: str):
        ...
        QMessageBox.warning(self, "URL 오류", err)
    ...
    def _on_queue_double_clicked(self, item: QListWidgetItem):
        ...
        if idx is None or idx >= len(self._queue):
            return
        webbrowser.open(self._queue[idx]['url'])
    ...
    def download_all(self):
        ...
        if not items:
            QMessageBox.information(self, "다운로드", "대기열이 비어있습니다.")
            return

        video = self.fmt_video.isChecked()
        audio = self.fmt_audio.isChecked()
        if not video and not audio:
            QMessageBox.warning(self, "형식 선택", "비디오 또는 오디오 중 하나 이상을 선택해주세요.")
            return

        dest = self.dest_input.text().strip()
        if not dest:
            QMessageBox.warning(self, "다운로드", "저장 경로를 설정해주세요.")
            return
        ...
        if len(items) == 1:
            custom = self.rename_input.text().strip()
            if custom:
                items[0]['custom_name'] = custom
...
def main():
    ...
    # Generate the checkbox icons before applying the QSS so the indicator
    # url(...) references resolve to real files on disk.
